...
def initialTerritoryScore():
    ...
    for i in territories.keys():
        score = 0
        if territories[i]["supply"]:
            score += 300
        for neighbor in paths[i]:
            if territories[neighbor]["supply"]:
                score += 150
            for neighborNeighbor in paths[i]:
...
def equalMoves(move1, move2):
    if move1["type"] != move2["type"]: return False
    for i in range(len(move1["terr"])):
        if move1["terr"][i] != move2["terr"][i]: return False
    return True
